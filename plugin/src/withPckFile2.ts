 ...
 */
const withPckFiles2: ConfigPlugin = (config) => {
  return withXcodeProject(config, async (config) => {
    const projectRoot = config.modRequest.projectRoot;
    const project = config.modResults;

    // Source path: assets/godot/main.pck
    const sourcePath = path.join(projectRoot, "assets", "godot", "peggle.pck");

    // Destination path: ios/main.pck (at the root of ios folder)
    const destPath = path.join(projectRoot, "ios", "peggle.pck");

    // Check if source file exists
    if (!fs.existsSync(sourcePath)) {
      console.warn("peggle.pck not found at assets/godot/peggle.pck");
      return config;
    }

    // Copy the file
    fs.copyFileSync(sourcePath, destPath);
    console.log("Copied peggle.pck to iOS project");

    // Check if file already exists in the project
    const existingFile = project.hasFile("peggle.pck");
    if (existingFile) {
      console.log("peggle.pck already exists in Xcode project");
      return config;
    }
    ...
    const file = project.addFile("peggle.pck", mainGroupKey, {
